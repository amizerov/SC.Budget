...
namespace SC.Zakup.Models
{
	public class CDetails : List<CDetail>
	{
		public CDetails()
		{
			DataTable dt = G.db_select("select ID, Name from Detail where IsActual=1 order by Name");
	...
	public class CDetail
	{
		...
		public void Update()
		{
			DataTable dt = G.db_select("select * from Detail where ID = {1}", ID);
			foreach (DataRow r in dt.Rows)
			{
				ID = G._I(r["ID"]);
				Name = G._S(r["Name"]);
		...
		public void Change(string name)
		{
			...
			string sql = @"
                declare @id int
                select @id = ID from Detail where [Name] = '{1}'
                if @id is null begin
                    insert Detail(Name) values('{1}')
                    set @id = @@IDENTITY
                end
                select @id 
            ";
