...
where s.ID = @id
	or (
		...
		and (@oplataStart is null or exists (select * from Oplata o where o.Schet_ID = s.ID and o.Data >= @oplataStart))
		and (@oplataEnd is null or exists (select * from Oplata o where o.Schet_ID = s.ID and o.Data < @oplataEnd))
		)
