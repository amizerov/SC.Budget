...
namespace am.BL
{
	public class G
	{
		...
		{
			get { return _err; }
			set { _err = value; if (_err.Length > 0) { if (OnError != null) OnError.Invoke(_err); } }
		}
		...
		{
			var par2 = par.Select(p => p == null ? "NULL" : p.ToString()).ToArray();
		...
		public static long _L(object p, int def)
		{
			long r = def;
			try
			{
		...
		public static long _L(DataTable d, string f)
		{
			long r = 0;
			try
			{
		...
		public static double _Double(object p)
		{
			return _Double(p, 0);
		}
		public static double _Double(object p, double def)
		{
			double r = def;
			try
			{
				...
				{
					DataTable dt = (DataTable)p;
					if (dt.Rows.Count > 0) r = double.Parse(dt.Rows[0][0] + "");
				}
				else if (p.GetType().Name == "DataRow")
				{
					DataRow dr = (DataRow)p;
		...
		public static long? _NL(DataTable d, string f)
		{
			long? r = null;
			try
			{
		...
		public static int? _NI(DataTable d, string f)
		{
			int? r = null;
			try
			{
		...
		public static decimal _Dec(object p, decimal def)
		{
			decimal r = def;
			try
			{
				...
				{
					DataTable dt = (DataTable)p;
					if (dt.Rows.Count > 0) r = decimal.Parse(dt.Rows[0][0] + "");
				}
				else if (p.GetType().Name == "DataRow")
				{
					DataRow dr = (DataRow)p;
					r = decimal.Parse(dr[0] + "");
				}
				else
				{
					if (p.ToString().Length > 0)
						r = decimal.Parse(p.ToString());
				}
			}
			catch
			{
		...
		public static decimal _Dec(object p)
		{
			return _Dec(p, default);
		}

		public static decimal? _NDec(object datarow)
		{
			decimal? r = null;
			try
			{
